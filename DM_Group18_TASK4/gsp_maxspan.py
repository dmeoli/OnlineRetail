...
#	  the span of s with respect to v is smaller than maxspan
# We say that a sequence s is a frequent-k-sequence if the support of s
# is higher then a threshold.
# We say that a sequence s is a candidate-k-sequence if every subsequence of 
...
def supports(mseq, cseq, maxspan):
    i = 0
...
def countSupport(dataset, cseq, maxspan):
	support_count = 0
	for seq in dataset:
...
def apriori_tc(dataset, maxspan, minsup, verbose=False):
    ...
    while (True):
    	...
    	candidatesCounts = [(i, countSupport(dataset, i, maxspan)) for i in candidatesPruned]
